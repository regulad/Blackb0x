...
private:
    ...
    // One full pass through the checkm8 exploit sequence — shells out to the
    // vendored `gaster` binary rather than driving the low-level USB
    // request sequence itself (see docs/HISTORY.md for why); checkm8() (public)
    // retries this a bounded number of times on failure — see its own
    // comment for why.
