...
// it. Not hand-rolled here per this repo's own "prefer adapting an
// existing, battle-tested library over hand-rolling the equivalent"
// convention (AGENTS.md) -- this is real-device-tested img3 tag surgery,
// worth reusing exactly as upstream wrote it.
//
// Changes from upstream, confirmed to be the only ones: dropped the
// idevicerestore.h/common.h includes (this project has neither, and
// nothing else in this file needs them) and every logger(LL_DEBUG, ...)/
// logger(LL_INFO, ...) call (this repo's own terse-output convention --
// see AGENTS.md's "don't reintroduce debug-dump printfs" rule); the two
// logger(LL_ERROR, ...) call sites became fprintf(stderr, ...) instead.
// No logic changed.
//
...
static img3_file* img3_parse_file(const unsigned char* data, unsigned int size) {
	...
	if (header->signature != kImg3Container) {
		fprintf(stderr, "img3_parse_file: Invalid IMG3 file\n");
		return NULL;
	}

	img3_file* image = (img3_file*) malloc(sizeof(img3_file));
	if (image == NULL) {
		fprintf(stderr, "img3_parse_file: Unable to allocate memory for IMG3 file\n");
		return NULL;
	}
	...
	if (image->header == NULL) {
		fprintf(stderr, "img3_parse_file: Unable to allocate memory for IMG3 header\n");
		img3_free(image);
		return NULL;
	}
	...
	while (data_offset < size) {
		...
		case kKbagElement:
			...
			if (element == NULL) {
				fprintf(stderr, "img3_parse_file: Unable to parse element (type 0x%08x)\n", current->signature);
				img3_free(image);
				return NULL;
			}
		...
		case kEcidElement:
			element = img3_parse_element(&data[data_offset]);
			if (element == NULL) {
				fprintf(stderr, "img3_parse_file: Unable to parse ECID element\n");
				img3_free(image);
				return NULL;
			}
		...
		case kShshElement:
			element = img3_parse_element(&data[data_offset]);
			if (element == NULL) {
				fprintf(stderr, "img3_parse_file: Unable to parse SHSH element\n");
				img3_free(image);
				return NULL;
			}
		...
		case kCertElement:
			element = img3_parse_element(&data[data_offset]);
			if (element == NULL) {
				fprintf(stderr, "img3_parse_file: Unable to parse CERT element\n");
				img3_free(image);
				return NULL;
			}
		...
		case kUnknElement:
			element = img3_parse_element(&data[data_offset]);
			if (element == NULL) {
				fprintf(stderr, "img3_parse_file: Unable to parse UNKN element\n");
				img3_free(image);
				return NULL;
			}
			image->elements[image->num_elements++] = element;
		...
		default:
			fprintf(stderr, "img3_parse_file: Unknown IMG3 element type %08x\n", current->signature);
			img3_free(image);
			return NULL;
		}
...
static img3_element* img3_parse_element(const unsigned char* data) {
	...
	if (element == NULL) {
		fprintf(stderr, "img3_parse_element: Unable to allocate memory for IMG3 element\n");
		return NULL;
	}
	...
	if (element->data == NULL) {
		fprintf(stderr, "img3_parse_element: Unable to allocate memory for IMG3 element data\n");
		free(element);
		return NULL;
	}
...
static int img3_replace_signature(img3_file* image, const unsigned char* signature) {
	...
	if (ecid == NULL || ecid->type != kEcidElement) {
		fprintf(stderr, "img3_replace_signature: Unable to find ECID element in signature\n");
		return -1;
	}
	...
	if (shsh == NULL || shsh->type != kShshElement) {
		fprintf(stderr, "img3_replace_signature: Unable to find SHSH element in signature\n");
		return -1;
	}
	...
	if (cert == NULL || cert->type != kCertElement) {
		fprintf(stderr, "img3_replace_signature: Unable to find CERT element in signature\n");
		return -1;
	}
...
static int img3_get_data(img3_file* image, unsigned char** pdata, unsigned int* psize) {
	...
	if (data == NULL) {
		fprintf(stderr, "img3_get_data: Unable to allocate memory for IMG3 data\n");
		return -1;
	}
	...
	if (offset != size) {
		fprintf(stderr, "img3_get_data: Incorrectly sized image data\n");
		free(data);
		*pdata = 0;
		*psize = 0;
		return -1;
...
int img3_stitch_component(const char* component_name, const void* component_data, size_t component_size, const void* blob, size_t blob_size, void** img3_data, size_t *img3_size)
{
	...
	if (img3 == NULL) {
		fprintf(stderr, "img3_stitch_component: Unable to parse %s IMG3 file\n", component_name);
		return -1;
	}

	if (((img3_element_header*)blob)->full_size != blob_size) {
		fprintf(stderr, "img3_stitch_component: Invalid blob passed for %s IMG3: The size %d embedded in the "
		                "blob does not match the passed size of %zu\n",
	...
	if (img3_replace_signature(img3, blob) < 0) {
		fprintf(stderr, "img3_stitch_component: Unable to replace %s IMG3 signature\n", component_name);
		img3_free(img3);
		return -1;
	}
	...
	if (img3_get_data(img3, &outbuf, &outsize) < 0) {
		fprintf(stderr, "img3_stitch_component: Unable to reconstruct %s IMG3\n", component_name);
		img3_free(img3);
		return -1;
	}
